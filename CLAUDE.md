...
- **2眼感知系统**: 左眼(-45°)、右眼(+45°)，半角60°，视觉半径150
- **16维输入**: 2眼×7通道（食物距离、同族距离、异族距离、同族能量、异族能量、同族体温、异族体温）+ 自身能量 + 体温状态
...
2. 2眼感知: `world.rs` → `compute_eye_perception()` 每帧计算（2眼×7通道）
3. 动作执行: `world.rs` → `execute_actions()` 4输出映射
4. 战力系统: `config.rs` → `combat_power()` 公式，`world.rs` → 咬时双方战力比对
5. 空间索引使用 FxHashMap，查询复用缓冲区避免分配
6. 种族聚类: `calculate_clan_cache()` 每秒更新一次
7. 所有复杂行为应是进化结果，避免硬编码
