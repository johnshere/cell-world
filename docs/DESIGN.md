...
| 7    | 发光强度     | tanh -1~1 | 26    | (v+1)/2→0~1，量化一位小数；渲染为身体正中白色小方块 |
...
```
 ...
 │                       BLOCK — 感官区 (|b| ≤ 7)                              │
 │                                                                             │
 │     Block -1 (左眼)        Block +1 (右眼)        Block 0 (体感)            │
 │     ┌──────────┐           ┌──────────┐           ┌──────────┐              │
 │     │Processing│           │Processing│           │Processing│              │
 │     │  Output  │           │  Output  │           │  Output  │              │
 │     └────┬─────┘           └────┬─────┘           └────┬─────┘              │
 │          │                      │                      │                    │
 │     Block -2 (发光感知)          │                 Block +2~+7 (预留)         │
 │                                                        │                    │
 └──────────┼──────────────────────┼──────────────────────┼────────────────────┘
            │                      │                      │
            │  ┌───────────────────┼──────────────────────┘
 ...
 │                    BLOCK — 联合区 (8 ≤ |b| ≤ 24)                            │
 │                                                                             │
 │  左半球 Block -8 ~ -24              右半球 Block +8 ~ +24                   │
 │  ┌──────────────────────┐           ┌──────────────────────┐                │
 │  │ Processing + Output  │◄─────────►│ Processing + Output  │                │
 │  │ (演化产生隐藏节点)    │  跨半球    │ (演化产生隐藏节点)    │                │
 │  └──────────┬───────────┘  (低概率)  └──────────┬───────────┘                │
 │             │                                   │                           │
 │  Block迁移变异: 联合区节点 block 编号可 ±2 漂移 (限 8~24)                    │
 │  is_same_side: Block 0 视为与任一侧同侧                                     │
 ...
 │                     BLOCK — 运动区 (25 ≤ |b| ≤ 31)                          │
 │                                                                             │
 │  Block 25 (运动)    Block -25 (繁殖)    Block 26 (发光)                      │
 │  ┌──────────────┐   ┌──────────────┐   ┌──────────────┐                     │
