...
- **左眼**：朝向 heading - 45°，FOV 半角 60°（覆盖 -105° ~ +15°）
- **右眼**：朝向 heading + 45°，FOV 半角 60°（覆盖 -15° ~ +105°）
- **视觉半径**：固定 150 单位
- **距离编码**：接近度 = `1.0 - dist / vision_range`（越近越大）
- **前方重叠**：±15° 范围内左右眼都能看到，提供立体感知
...
[14]    自身能量 (0~1, energy / 200)
[15]    体温状态 (0~1, 冷却时长 / 100，越冷越高)
```

每只眼跟踪最近的食物/同族/异族目标：
- 距离通道：接近度（0=无目标，1=紧贴）
- 能量通道：最近目标的能量归一化值（0~1）
- 体温通道：最近目标的冷却程度（0=暖，1=极冷）
...
struct Creature {
    ...
    current_speed: f64,           // 当前速度（战力计算用）
    perception_cache: [f64; 16],  // 感知缓存（2眼×7通道+能量+体温）
}
```
...
条件：嘴输出 < -0.1，且接触到其他生物
流程：
  1. 计算双方战力：combat_power = (energy/100) × (1+温度加成) × (1+速度加成) × (1+同族援助加成)
     - 温度加成：combat_temp_weight × warmth（暖=1, 冷=0）
     - 速度加成：combat_speed_weight × (current_speed / 50)
     - 同族援助：combat_ally_weight × (附近同族总能量 / 500).min(1)
  2. 战力比：power_ratio = my_power / (my_power + other_power)
  3. 咬伤量：target_energy × bite_strength × 0.2 × power_ratio × 2
  4. 消化效率：1.0 - similarity（异族好消化，同族难消化）
效果：等战力=原基准；强打弱最多翻倍；弱打强接近无效
      群体围攻时同族援助大幅提升战力
```
...
```
条件：嘴输出 > +0.1，且接触到其他生物
效果：转移自身 20% 能量给目标，固定效率 50%（无体型限制）
      被哺育方回暖 70%（远强于吃食物的 20%）
```
...
pub struct Config {
    ...
    pub feed_efficiency: f64,              // 0.5 (喂食效率50%)
    ...
    // 战力系统
    pub combat_temp_weight: f64,       // 0.5 (暖体温+50%战力)
    pub combat_speed_weight: f64,      // 0.3 (满速+30%战力)
    pub combat_ally_weight: f64,       // 0.8 (同族满援+80%战力)
    pub combat_ally_range: f64,        // 50.0 (援助感应范围)
...
- [x] NEAT 基因组（节点 + 连接）
- [x] 结构进化（新增节点、连接、权重变异）
- [x] 2眼感知系统（16维输入：2眼×7通道+能量+体温）
- [x] 4输出动作（转向、速度、嘴、繁殖）
- [x] 战力系统（咬伤量由能量/体温/速度/同族援助决定）
- [x] 有性/无性繁殖
- [x] 火山 + 陨石能量系统
- [x] 祖先追溯种族聚类
- [x] 优势种自动保存
