...
- **海底火山地形**：可由用户一次性生成的 fBm 噪声高度图（50×50 chunk，火山圆锥+多倍频噪声+种子），影响生物移动消耗（上坡阻力），生成后冻结
...
右眼 [8..15]: 结构同左眼对称                                   → block +1

自身能量 [16]: (energy / 2000).min(1)                         → block -3 (内省)

地形感知 [17]: 前方坡度方向 dh.signum() {-1=下坡, 0=平地, 1=上坡}  → block +3 (外感觉)
            前瞻 15px，地形未生成时恒为 0

光语言 [18..19]:                                              → block -2 ("光耳")
  [18] 扫描角度归一化  (-1 ~ 1)
  [19] 发光强度       最近发光生物的 light_intensity (0~1)
```

> 常量来源：`Genome::INPUT_SIZE = 20`，写入逻辑见 `src/world/world.rs::compute_perception_pure`。

### 输出（8 维，固定，全部直读 tanh）

| 编号 | 功能         | 输出范围   | Block | 说明 |
|------|--------------|-----------|-------|------|
| 0    | 转向角       | tanh -1~1 | +25   | × 2π = 每秒转向弧度（最大每秒一圈） |
| 1    | 速度         | tanh -1~1 | +25   | abs × `max_speed` = 每秒移动距离 |
| 2    | 嘴           | tanh -1~1 | +25   | < -0.1 咬（攻击），冷却 1s；接触食物自动吸收（不受冷却限制） |
| 3    | 繁殖意愿     | tanh -1~1 | -25   | > 0.2 时触发繁殖 |
| 4    | 繁殖阈值     | tanh→sig  | -25   | 映射到 20~200 能量阈值 |
| 5    | 子代能量比例 | tanh→sig  | -25   | 映射到 0.1~0.5 |
| 6    | 痕迹强度     | tanh -1~1 | +25   | 取正半轴 → 0~0.3，作为自身能量比例额外投放痕迹 |
| 7    | 发光强度（光嘴）| tanh -1~1 | -26 | (v+1)/2→0~1，量化一位小数；跟光耳 -2 同侧成完整语言闭环 |

> 常量来源：`Genome::OUTPUT_SIZE = 8`，执行逻辑见 `src/world/world.rs::execute_actions`。
> 喂食机制已移除，能量分享通过痕迹点实现。
...
```
 ...
 │                              INPUT (20维固定)                               │
 │                                                                             │
 │  左眼 [0..7]         右眼 [8..15]       体感 [16..17]     发光感知 [18..19] │
 │  ┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐     │
 │  │ 0 扫描角归一化    │     │ 8 扫描角归一化    │     │16 自身能量/2000  │     │
 │  │ 1 目标接近度      │     │ 9 目标接近度      │     │17 前方坡度方向   │     │
 │  │ 2 目标能量/200    │     │10 目标能量/200    │     └────────┬─────────┘     │
 │  │ 3 实体类型        │     │11 实体类型        │              │               │
 │  │ 4 基因相似度      │     │12 基因相似度      │              │               │
 │  │ 5 FOV能量密度     │     │13 FOV能量密度     │              │               │
 │  │ 6 朝向差(仅生物)  │     │14 朝向差(仅生物)  │              │               │
 │  │ 7 速度差(仅生物)  │     │15 速度差(仅生物)  │              │               │
 │  └────────┬─────────┘     └────────┬─────────┘              │               │
 └───────────┼────────────────────────┼──────────────────────────┼───────────────┘
             │                        │                          │
             │ (硬约束:Input只连       │                          │
             ...
             ▼                        ▼                          ▼
 ┌─────────────────────────────────────────────────────────────────────────────┐
 │            BLOCK — 感官区 (|b| ∈ {1,2,3}，每种感官占一对镜像 abs)             │
 │                                                                             │
 │  视觉:        Block -1 (左眼)         Block +1 (右眼)                       │
 │  光语言:      Block -2 (光耳/输入)    Block +2 (对侧"听皮层"/无input)        │
 │  内省/外感:   Block -3 (自身能量)     Block +3 (地形)                       │
 │  ┌───────────────────────────────┬───────────────────────────────────┐      │
 │  │ Processing + Output           │ Processing + Output               │      │
 │  │ (既接收 input，又允许内部处理) │ (类比 V1/A1)                       │      │
 │  └────────────────┬──────────────┴───────────────────┬───────────────┘      │
 │                   │                                  │                       │
 │  跨半球前馈受同源约束: 仅允许 |from|==|to| (左 V1↔右 V1)                     │
 │  block 0: 永久弃用，无 input/output 投射                                     │
 └───────────────────┼──────────────────────────────────┼──────────────────────┘
            │                      │                      │
            │  ┌───────────────────┼──────────────────────┘
            │  │  ConnProbs 5方向概率控制:
            │  │    同区Proc / 同区Output / 跨区前馈同侧 / 跨区前馈对侧 / 跨区反馈
            │  │  + target_pref 目标block偏好倍率 (可演化)
            ▼  ▼
 ┌─────────────────────────────────────────────────────────────────────────────┐
 │                  BLOCK — 联合区 (4 ≤ |b| ≤ 24，演化产生)                    │
 │                                                                             │
 │  左半球 Block -4 ~ -24              右半球 Block +4 ~ +24                   │
 │  ┌──────────────────────┐           ┌──────────────────────┐                │
 │  │ Processing + Output  │◄─同源──►  │ Processing + Output  │                │
 │  │ (演化产生隐藏节点)    │  跨半球    │ (演化产生隐藏节点)    │                │
 │  └──────────┬───────────┘  仅同 abs   └──────────┬───────────┘                │
 │             │                                   │                           │
 │  Block迁移变异: 联合区节点 block 编号可 ±2 漂移 (限 1~24，含感官)            │
 │  is_same_side: 严格按符号 (a>0)==(b>0)，0 已弃用无中轴特权                   │
 │  跨半球前馈: CrossForwardOther 必须 |from|==|to| (仿胼胝体)                  │
 │  前馈方向: |target_block| > |source_block|                                  │
 └─────────────┼───────────────────────────────────┼───────────────────────────┘
               │                                   │
               │  (硬约束: Output节点只从运动区接收)  │
               ▼                                   ▼
 ...
 │                     BLOCK — 运动区 (25 ≤ |b| ≤ 31)                          │
 │                                                                             │
 │  Block +25 (运动)   Block -25 (繁殖)    Block -26 (光嘴/语言)                │
 │  ┌──────────────┐   ┌──────────────┐   ┌──────────────┐                     │
 │  │ Output层     │   │ Output层     │   │ Output层     │                     │
 │  └──────┬───────┘   └──────┬───────┘   └──────┬───────┘                     │
 └─────────┼──────────────────┼──────────────────┼─────────────────────────────┘
           │                  │                  │
           ▼                  ▼                  ▼
 ...
 │                            OUTPUT (8维固定)                                  │
 │                                                                             │
 │  Block +25:                   Block -25:                  Block -26:         │
 │  ┌──────────┬─────┬────────┐ ┌──────────┬──────┬───────┐ ┌───────────────┐  │
 │  │0 转向角  │1速度│2 嘴    │ │3繁殖意愿 │4阈值 │5比例  │ │7 发光强度     │  │
 │  │tanh×2π  │×spd │<-0.1咬 │ │>0.2触发  │20~200│0.1~0.5│ │(v+1)/2→0~1   │  │
 │  ├──────────┘     │冷却1s  │ │          │      │       │ │量化一位小数   │  │
 │  │6 痕迹强度       │吸收食物│ │          │      │       │ │              │  │
 │  │正半轴→0~30%能量 │       │ │          │      │       │ │              │  │
 │  └────────────────┴────────┘ └──────────┴──────┴───────┘ └───────────────┘  │
 ...
 连接规则:
   Input ──────► 只连同 block 感官区 (硬约束)
   Block(感官)──► 同区内 / 跨区前馈 / 跨区反馈 (ConnProbs 概率控制)
   Block(联合)──► 同上，联合区间可互连，block可漂移
   Block(运动)──► Output (对外投射)
   Output ◄───── 只从运动区 Block 接收 (硬约束)

 跨半球同源约束 (CrossForwardOther 仅同 |abs|):
   左 V1 (-1) ↔ 右 V1 (+1)         允许
   左 联合 12 (-12) ↔ 右联合 12 (+12)  允许
   左联合 12 → 右联合 24            禁止 (跨级跨半球)
   左 V1 → 右运动 (-1 → +25)        禁止 (跨级跨半球)
   仿胼胝体镜像拓扑，跨半球对接需对侧存在同 abs 节点

 每个 Block 节点有两层:
   Processing: 内部处理，区内递归
   Output:     对外投射层，跨区信号出口

 ConnProbsGene (每个 block 独立，可演化):
   P[5]: Processing层5方向概率
   Q[5]: Output层5方向概率
   target_pref: 目标block偏好倍率 (稀疏HashMap, >1偏爱 <1回避)
```
...
struct Genome {
    ...
    conn_probs: HashMap<i8, ConnProbsGene>,  // -24..=24 每个 block 的连接概率基因
...
| Block 迁移 | block | 联合区节点 block 编号 ±2 移动（限制在 1~24，含感官） |
...
lava_spread_probability = 0.5         # 兼任两职：①熔岩死亡时分裂子粒子的概率 ②普通粒子落地簇生时每次伯努利掷骰的概率（每个落点掷 10 次，命中则在落点周围梭形区域（长 200 / 短 20，长轴沿火山→落点的径向）内追加同能量普通粒子，不杀伤、不再扩散）
...
- [x] **熔岩流系统**：火山喷发附带链式扩散粒子，溢流机制（等效高度=地形+系数×(2^n-1)，n=区块内所有存活粒子数，实时更新计数），子粒子初始落入父区块后沿液面梯度溢流（visited防回弹+max_overflow_depth），独立衰减率，周期性杀伤关联扩散代数（depth_ratio=chain_depth/max_chain_depth，代数越深杀伤越弱）
